...
import { collection, addDoc, serverTimestamp, query, doc, deleteDoc, getDocs } from 'firebase/firestore';
...
export default function EventsPage() {
  ...
  // A just-created event has a null serverTimestamp until the server confirms,
  // and Firestore excludes docs missing the ordered field — which made new
  // events briefly disappear. We fetch unordered and sort client-side instead,
  // treating pending (null createdAt) events as newest so they show instantly.
  ...
  const toMillis = (createdAt: any): number => {
    if (!createdAt) return Number.MAX_SAFE_INTEGER; // pending local write -> newest
      ...
      <html>
        <head>
          ...
          <style>
            ...
            h1 { margin: 0; color: #7B1E2B; font-size: 32px; font-weight: 800; line-height: 1.2; }
            p { font-size: 18px; color: #4b5563; margin: 10px 0; }
            .logo { font-weight: 800; font-size: 24px; color: #7B1E2B; margin-bottom: 24px; letter-spacing: -0.5px; }
            .instruction { font-weight: bold; color: #7B1E2B; border-top: 2px solid #f3f4f6; padding-top: 20px; margin-top: 20px; font-size: 20px; }
